...
#SBATCH --mem=4G ## how much RAM do you need per computer/node (this affects your FairShare score so be careful to not ask for more than you need))
#SBATCH --job-name=2D_disk ## When you run squeue -u
#SBATCH --mail-type=ALL
#SBATCH --mail-user=jupiterding2029@u.northwestern.edu

export OMP_NUM_THREADS=8 #32 ## number of cores used per job
...
# configure problem
python ~/athena/configure.py --prob=disk --coord=cylindrical --eos=isothermal -hdf5 -h5double -mpi --hdf5_path /hpc/software/spack_v20d1/spack/opt/spack/linux-rhel7-x86_64/intel-2021.4.0/hdf5-1.10.7-exmishiaff3ugqbeeimkqvnwrir76sxh -omp --cflag=-qno-openmp-simd
make clean
make -j

# run the sim
~/athena/bin/athena -i athinput.disk_cyl
